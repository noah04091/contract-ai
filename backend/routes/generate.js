...
// ============================================
// 🆕 AUTOMATISCHE PDF-GENERIERUNG UND S3-UPLOAD
// ============================================
/**
 * Generiert PDF aus HTML und lädt es automatisch zu S3 hoch
 * Wird NACH dem Speichern des Vertrags asynchron aufgerufen
 * @param {string} contractId - MongoDB ObjectId des Vertrags
 * @param {string} userId - User ID
 * @param {string} htmlContent - HTML-Inhalt für PDF
 * @param {string} contractName - Name des Vertrags für Dateiname
 */
const generatePDFAndUploadToS3 = async (contractId, userId, htmlContent, contractName) => {
  console.log(`🚀 [AUTO-PDF] Starte automatische PDF-Generierung für Contract: ${contractId}`);

  let browser = null;

  try {
    // 1. Puppeteer Browser starten
    if (chromium) {
    ...
    console.log(`✅ [AUTO-PDF] Browser gestartet`);
    ...
    // 3. Zu S3 hochladen (mit AWS SDK v3)
    ...
    console.log(`📤 [AUTO-PDF] Uploading to S3: ${s3Key}`);
    ...
    console.log(`✅ [AUTO-PDF] S3 Upload erfolgreich: ${s3Key}`);

    // 4. Contract in MongoDB mit s3Key aktualisieren
    ...
    console.error(`❌ [AUTO-PDF] Fehler bei automatischer PDF-Generierung:`, error.message);
    ...
    if (browser) {
      try {
        ...
        console.error(`❌ [AUTO-PDF] Browser konnte nicht geschlossen werden:`, closeError.message);
      }
    }
...
router.post("/", verifyToken, async (req, res) => {
  ...
  if (shouldUseV2) {
    ...
    try {
      ...
      // 🆕 AUTO-PDF: Asynchron PDF generieren und zu S3 hochladen (blockiert nicht die Response)
      generatePDFAndUploadToS3(
        ...
        if (pdfResult.success) {
          console.log(`✅ [V2] Auto-PDF erfolgreich: ${pdfResult.s3Key}`);
        } else {
          console.error(`❌ [V2] Auto-PDF fehlgeschlagen:`, pdfResult.error);
        }
      }).catch(err => {
        console.error(`❌ [V2] Auto-PDF Exception:`, err);
      });
...
DAS IST KEIN "Vertrag neu schreiben" - DAS IST "Vertrag gezielt verbessern"!`;
    ...
    generatePDFAndUploadToS3(
      ...
      if (pdfResult.success) {
        console.log(`✅ [V1] Auto-PDF erfolgreich: ${pdfResult.s3Key}`);
      } else {
        console.error(`❌ [V1] Auto-PDF fehlgeschlagen:`, pdfResult.error);
      }
    }).catch(err => {
      console.error(`❌ [V1] Auto-PDF Exception:`, err);
    });
