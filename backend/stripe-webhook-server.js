  ...
  switch (event.type) {
    ...
    case "customer.subscription.deleted": {
      ...
      try {
        ...
        // 🎟️ 20.08.2026: Persönlicher Code statt des pauschalen "COMEBACK20". Der alte
        // Code war unbefristet und unbegrenzt einlösbar, obwohl die Mail "einmalig, nur
        // für dich, 14 Tage gültig" behauptete. Jetzt stimmt beides.
        // Wirft nie: Kommt kein Code zurück, geht die Mail ohne Angebot raus.
        ...
        // Am Konto merken, damit die Follow-up-Mail 3 Tage später DENSELBEN Code nennt
        // und der Kunde nicht zwei verschiedene bekommt.
        if (angebot?.code) {
          await usersCollection.updateOne(
            { _id: user._id },
            { $set: { retentionOffer: {
        ...
        const rabattCode = angebot?.code || null;
        ...
        await sendEmail({
          ...
          html: generateEmailTemplate({
            ...
            preheader: rabattCode
              ? "Bevor du gehst: 20% Rabatt für 3 Monate, nur für dich. Deine Verträge bleiben gespeichert."
              : "Deine Verträge und Daten bleiben gespeichert.",
            body: `
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="margin: 0 0 20px 0; background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 12px;">
                <tr>
                  <td style="padding: 24px; text-align: center;">
                    <div style="font-size: 11px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; color: #1d4ed8;">Einmalig, nur f&uuml;r dich${gueltigBis ? ` &middot; g&uuml;ltig bis ${gueltigBis}` : ''}</div>
