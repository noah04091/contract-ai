    ...
    if (hasValidExpiry && shouldCreateCriticalEvents) { // 🔒 Konfidenz-Check + echtes Enddatum (Problem-A-Guard)
      ...
      if (noticePeriodDays > 0 && hasCancelReason) {
        ...
        if (cancelWindowDate > now) {
          events.push({
            ...
            description: `Ab jetzt solltest du dich um die Kündigung von "${contract.name}" kümmern. Letzte Möglichkeit: ${deadlineStr}. Kündigungsfrist: ${noticePeriodLabel}.${isAutoRenewal ? ' (Auto-Renewal Vertrag)' : ''}`,
        ...
        if (lastCancelDate > now) {
          ...
          if (warningDate > now) {
            events.push({
              ...
              description: `In 7 Tagen endet die Kündigungsfrist für "${contract.name}". ${isAutoRenewal ? 'Auto-Renewal Vertrag - jetzt handeln!' : 'Jetzt handeln!'}`,
              date: warningDate,
              severity: "warning",
              status: "scheduled",
              confidence: confidence,
      ...
      if (oneYearFromCreation > now && oneYearFromCreation < expiryDate) {
        events.push({
          ...
          description: `Zeit für einen Check: Ist "${contract.name}" noch optimal für dich? ${isAutoRenewal ? 'Dieser Vertrag verlängert sich automatisch.' : 'Vergleiche Alternativen!'}`,
      ...
      if (gewaehrleistungDate > now) {
        ...
        if (reminderDate > now) {
          events.push({
            ...
            description: `In 30 Tagen endet die Gewährleistung für "${contract.name}". Prüfe, ob alles in Ordnung ist!`,
            date: reminderDate,
            severity: "info",
            status: "scheduled",
            confidence: confidence,
