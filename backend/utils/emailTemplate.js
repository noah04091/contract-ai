...
// ✅ V10 - KEINE Template-Literals! Nur String-Concatenation + Replace

/**
 * Generiert E-Mail HTML basierend auf dem clean3 Template
 * WICHTIG: Diese Version verwendet KEINE Template-Literals (Backticks)
 * um Spam-Filter-Probleme zu vermeiden!
 */
function generateEmailTemplate(options) {
  var title = options.title || '';
  var body = options.body || '';
  var cta = options.cta || null;
  var badge = options.badge || 'Erinnerung';

  // CTA Button HTML - ohne Template-Literals!
  var ctaHtml = '';
  ...
  // Das EXAKTE HTML von clean3 - 100% hardcoded!
  var html = [
    '<!DOCTYPE html>',
    '<html lang="de">',
    '<head>',
...
function generateInfoBox(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (isFirst && !isLast) {
      ...
      style = 'border-top: 1px solid #e2e8f0; padding-top: 16px; padding-bottom: 16px;';
    }
 ...
 * OHNE Template-Literals!
 */
function generateAlertBox(text, type) {
  type = type || 'warning';
 ...
 * OHNE Template-Literals!
 */
function generateParagraph(text, style) {
  style = style || 'color: #374151; font-size: 16px; line-height: 1.6; margin-bottom: 16px;';
  return '<p style="' + style + '">' + text + '</p>';
}

module.exports = {
  generateEmailTemplate: generateEmailTemplate,
  generateInfoBox: generateInfoBox,
