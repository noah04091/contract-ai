...
(async () => {
  ...
  const ids = [...new Set([...(await evCol.distinct('userId')), ...(await cCol.distinct('userId'))].map(String))];
  ...
  console.log(`Zu löschen: ${evToDelete.length} Events, ${cToDelete.length} Verträge`);
  ...
  fs.writeFileSync(backupFile, JSON.stringify({ orphanIds, events: evToDelete, contracts: cToDelete }, null, 2));
  console.log(`💾 Backup geschrieben: ${backupFile} (${evToDelete.length} Events + ${cToDelete.length} Verträge)`);
  ...
  console.log(`🗑️ Gelöscht: ${evRes.deletedCount} Events, ${cRes.deletedCount} Verträge.`);
