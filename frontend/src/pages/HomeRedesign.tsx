...
const Check = ({ stroke = "#1d4ed8", w = 18, sw = "2.4" }: { stroke?: string; w?: number; sw?: string }) => (
...
const VerifiedBadge = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="#1d4ed8"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20z"></path><path d="M9 12l2 2 4-4" stroke="#fff" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"></path></svg>
);
...
const voicesRow1: Voice[] = [
  ...
  { name: "Marcus T.", meta: "München · IT-Berater", initials: "MT", initialBg: "linear-gradient(135deg,#3b82f6,#1d4ed8)", verified: true, text: "„Endlich verstehe ich meine Mietverträge ohne Anwalt. Die KI erklärt alles verständlich — spart Zeit und Geld.“" },
...
const voicesRow2: Voice[] = [
  ...
  { name: "Kevin B.", meta: "Stuttgart · BWL-Student", initials: "KB", initialBg: "linear-gradient(135deg,#f59e0b,#b45309)", fourStar: true, text: "„Mein Fitnessstudio-Abo lief automatisch weiter. Contract AI hat mein Sonderkündigungsrecht erkannt — 360€ zurück!“" },
  ...
  <div style={s("width:340px;flex:none;background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:16px;padding:22px")}>
    <div style={s("display:flex;align-items:center;gap:12px;margin-bottom:14px")}>
      ...
      <div>
        ...
        <span style={s("font-size:12.5px;color:#71717a")}>{v.meta}</span>
      </div>
    </div>
...
const GRAD = "background:linear-gradient(100deg,#1d4ed8,#3b82f6 50%,#06b6d4);-webkit-background-clip:text;background-clip:text;color:transparent";
...
const HomeRedesign = () => {
  ...
  const showcase = [
    {
      reverse: false, eyebrowColor: "#1d4ed8", eyebrowBg: "rgba(37,99,235,0.07)", eyebrowBorder: "rgba(37,99,235,0.16)",
      ...
      linkColor: "#1d4ed8", linkLabel: "Zur Vertragsanalyse", to: "/features/vertragsanalyse",
      video: analyseVideo, poster: analyseImg, label: "Vertragsanalyse · score",
    },
    {
      reverse: true, eyebrowColor: "#1d4ed8", eyebrowBg: "rgba(29,78,216,0.07)", eyebrowBorder: "rgba(29,78,216,0.16)",
      ...
      linkColor: "#1d4ed8", linkLabel: "Zum Optimierer", to: "/features/optimierung",
      video: optimierungVideo, poster: optimierungImg, label: "Klausel §4 · Haftung",
    },
    {
      reverse: false, eyebrowColor: "#047857", eyebrowBg: "rgba(5,150,105,0.07)", eyebrowBorder: "rgba(5,150,105,0.16)",
      ...
      linkColor: "#047857", linkLabel: "Zum Fristenkalender", to: "/features/fristen",
  ...
  // 8 Feature-Karten
  const featureCards = [
    { to: "/features/vertragsanalyse", color: "#1d4ed8", bg: "rgba(37,99,235,0.09)", title: "Analyse", desc: "Verträge KI-basiert auswerten, Risiken erkennen & Transparenz gewinnen.", link: "Vertragsanalyse entdecken", icon: <><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line><line x1="11" y1="8" x2="11" y2="14"></line><line x1="8" y1="11" x2="14" y2="11"></line></> },
    { to: "/features/optimierung", color: "#1d4ed8", bg: "rgba(29,78,216,0.09)", title: "Optimierung", desc: "Unvorteilhafte Klauseln erkennen und durch bessere Formulierungen ersetzen.", link: "Optimierung entdecken", icon: <path d="M15 4V2M15 16v-2M8 9h2M20 9h2M17.8 11.8 19 13M17.8 6.2 19 5M3 21l9-9M12.2 6.2 11 5"></path> },
    { to: "/features/fristen", color: "#b45309", bg: "rgba(217,119,6,0.10)", title: "Fristen", desc: "Kündigungsfristen automatisch erkennen & rechtzeitig per Mail erinnert werden.", link: "Fristenkalender entdecken", icon: <><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></> },
    { to: "/features/vergleich", color: "#0369a1", bg: "rgba(3,105,161,0.09)", title: "Vergleich", desc: "Zwei Verträge gegenüberstellen, Unterschiede visuell hervorheben.", link: "Vertragsvergleich entdecken", icon: <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></> },
    { to: "/features/legal-lens", color: "#0e7490", bg: "rgba(8,145,178,0.10)", title: "Legal Lens", desc: "Interaktive Vertragsanalyse mit Risiko-Score, Smart Summary & Verhandlungs-Empfehlungen.", link: "Legal Lens entdecken", icon: <><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"></path><circle cx="12" cy="12" r="3"></circle></> },
    { to: "/features/generator", color: "#047857", bg: "rgba(5,150,105,0.10)", title: "Generator", desc: "Vertragsdokumente aus KI-Vorlagen erzeugen, z. B. Freelancer-, NDA- oder Mietverträge.", link: "Vertragsgenerator entdecken", icon: <><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="12" y1="18" x2="12" y2="12"></line><line x1="9" y1="15" x2="15" y2="15"></line></> },
    { to: "/features/digitalesignatur", color: "#0f766e", bg: "rgba(13,148,136,0.10)", title: "Digitale Signatur", desc: "Verträge rechtssicher digital signieren, mit Audit Trail, Benachrichtigung & versiegeltem PDF.", link: "Digitale Signatur entdecken", icon: <><path d="M12 19l7-7 3 3-7 7-3-3z"></path><path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z"></path><path d="M2 2l7.586 7.586"></path><circle cx="11" cy="11" r="2"></circle></> },
    { to: "/features/legalpulse", color: "#b91c1c", bg: "rgba(220,38,38,0.09)", title: "Legal Pulse", desc: "Frühwarnsystem für neue Risiken durch Gesetzesänderungen oder unfaire Formulierungen.", link: "Legal Pulse entdecken", icon: <path d="M22 12h-4l-3 9L9 3l-3 9H2"></path> },
  ];
    ...
    <>
      ...
      <div className="ca-lp-page" ref={pageRef}>
        ...
        <section id="top" style={s("position:relative;padding:84px 24px 72px")}>
          <div style={s("position:absolute;inset:0;pointer-events:none;background:radial-gradient(620px 420px at 18% 2%,rgba(37,99,235,0.16),transparent 60%),radial-gradient(560px 420px at 88% 8%,rgba(37,99,235,0.14),transparent 60%),radial-gradient(560px 460px at 64% 96%,rgba(6,182,212,0.10),transparent 62%)")}></div>
          <div className="ca-lp-2col" style={s("position:relative;max-width:1200px;margin:0 auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(min(440px,100%),1fr));gap:52px;align-items:center")}>
            <div className="ca-lp-hero-col">
              <div style={s("display:inline-flex;align-items:center;gap:8px;padding:6px 12px 6px 8px;border:1px solid rgba(37,99,235,0.2);background:rgba(37,99,235,0.06);border-radius:999px;margin-bottom:24px")}>
                <span style={s("display:inline-flex;align-items:center;gap:5px;background:#1d4ed8;color:#fff;font-family:'Geist Mono',monospace;font-size:10.5px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;padding:3px 8px;border-radius:999px")}>Neu</span>
              ...
              <div style={s("margin:18px 0 0;height:52px")}>
                <svg viewBox="0 0 200 50" width="220" height="55" fill="none" style={{ overflow: 'visible' }} aria-label="Contract AI">
                  <path pathLength={1} d="M18,18 C8,20 4,28 6,36 C8,44 16,48 26,46 C30,45 33,43 35,40" stroke="#1d4ed8" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" style={s("stroke-dasharray:1;stroke-dashoffset:1;animation:caSigDraw .55s ease forwards .25s")}></path>
                  <path pathLength={1} d="M28,38 C32,36 35,32 38,34 C41,36 40,40 43,38 C46,36 48,32 52,34 C54,35 54,38 56,36 C58,34 60,30 64,32 C66,33 66,38 69,36 C72,34 74,30 78,32 C80,34 80,38 83,35 C86,32 88,28 92,32" stroke="#1d4ed8" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" style={s("stroke-dasharray:1;stroke-dashoffset:1;animation:caSigDraw .9s ease forwards .7s")}></path>
              ...
              <div className="ca-lp-hero-cta" style={s("display:flex;flex-wrap:wrap;gap:13px;margin-top:34px")}>
                <Link to={heroPrimaryTo} className="ca-lp-btn-primary" style={s("display:inline-flex;align-items:center;gap:8px;background:#1d4ed8;color:#fff;font-size:16px;font-weight:500;padding:15px 26px;border-radius:11px;text-decoration:none;box-shadow:0 1px 2px rgba(17,17,20,0.12)")}>{heroPrimaryLabel}<Arrow w={17} /></Link>
              ...
              <div style={s("display:inline-flex;align-items:center;gap:9px;margin-top:30px;padding:9px 15px;background:rgba(17,17,20,0.035);border:1px solid rgba(17,17,20,0.06);border-radius:10px")}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1d4ed8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
            ...
            <div style={s("position:relative")} className="ca-lp-reveal">
              <div style={s("position:relative;background:#ffffff;border:1px solid rgba(17,17,20,0.09);border-radius:18px;box-shadow:0 40px 80px -28px rgba(17,17,20,0.28),0 10px 26px rgba(17,17,20,0.06);overflow:hidden")}>
                <div style={s("height:44px;display:flex;align-items:center;gap:8px;padding:0 16px;border-bottom:1px solid rgba(17,17,20,0.07);background:#fbfbfd")}>
                  <span style={s("font-size:12.5px;color:#71717a;font-family:'Geist Mono',monospace")}>Vertragsanalyse · mietvertrag.pdf</span>
                </div>
                <div style={s("padding:20px;display:grid;grid-template-columns:1.15fr 1fr;gap:16px;background:#fafafb")}>
                  <div style={s("background:#fff;border:1px solid rgba(17,17,20,0.07);border-radius:12px;padding:16px;position:relative;overflow:hidden")}>
                    ...
                    <div style={s("font-size:11px;font-family:'Geist Mono',monospace;color:#71717a;letter-spacing:0.06em;text-transform:uppercase;margin-bottom:12px")}>Klauseln</div>
                    ...
                    <div style={s("height:7px;border-radius:4px;background:rgba(239,68,68,0.22);width:85%;margin-bottom:6px;position:relative")}>
                      <span style={s("position:absolute;right:-6px;top:-7px;font-size:9px;font-weight:600;color:#b91c1c;background:#fef2f2;border:1px solid #fecaca;border-radius:5px;padding:1px 5px")}>Risiko</span>
                    </div>
                  ...
                  <div style={s("display:flex;flex-direction:column;gap:12px")}>
                    <div style={s("background:#fff;border:1px solid rgba(17,17,20,0.07);border-radius:12px;padding:16px;display:flex;flex-direction:column;align-items:center;gap:10px")}>
                      <div style={s("width:104px;height:104px;border-radius:50%;background:conic-gradient(#1d4ed8 0 87%,#ece9f8 87% 100%);display:flex;align-items:center;justify-content:center")}>
                        <div style={s("width:78px;height:78px;border-radius:50%;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center")}>
                          ...
                          <span style={s("font-size:10px;color:#71717a;font-family:'Geist Mono',monospace")}>Score</span>
                        </div>
                      </div>
              ...
              <div style={s("position:absolute;left:-18px;bottom:30px;display:flex;align-items:center;gap:9px;background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:12px;padding:11px 14px;box-shadow:0 16px 34px -12px rgba(17,17,20,0.22);animation:caFloatY 5s ease-in-out infinite")}>
                ...
                <div><div style={s("font-size:13px;font-weight:600;color:#0c0c10;line-height:1.1")}>Analyse fertig</div><div style={s("font-size:11px;color:#71717a;font-family:'Geist Mono',monospace")}>in 58 Sek.</div></div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div style={s("max-width:1100px;margin:0 auto")} className="ca-lp-reveal">
            <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(220px,100%),1fr));gap:14px")}>
              {[
                ...
                { c: "#1d4ed8", t: "Serverstandort Frankfurt", icon: <><rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="M2 9h20"></path></> },
                { c: "#1d4ed8", t: "KI-gestützt & juristisch geprüft", icon: <path d="M12 2a3 3 0 0 0-3 3v1a3 3 0 0 0-3 3 3 3 0 0 0 0 6 3 3 0 0 0 3 3v1a3 3 0 0 0 6 0v-1a3 3 0 0 0 3-3 3 3 0 0 0 0-6 3 3 0 0 0-3-3V5a3 3 0 0 0-3-3z"></path> },
        ...
        <section id="features" style={s("padding:64px 24px 40px;position:relative;overflow:hidden;isolation:isolate")}>
          ...
          <div style={s("max-width:1200px;margin:0 auto")}>
            <div style={s("max-width:680px;margin:0 auto;text-align:center")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>So funktioniert's</span>
            ...
            <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(258px,100%),1fr));gap:18px;margin-top:48px")} className="ca-lp-stagger">
              {featureCards.map((f, i) => (
                <Link key={i} to={f.to} className="ca-lp-card" style={s("text-decoration:none;background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:16px;padding:26px;display:block")}>
                  ...
                  <span style={s("display:inline-flex;align-items:center;gap:6px;font-size:14px;font-weight:500;color:#1d4ed8")}>{f.link}<Arrow w={15} /></span>
                </Link>
              ))}
            </div>
            <div style={s("text-align:center;margin-top:36px")} className="ca-lp-reveal">
              ...
              <Link to="/features" className="ca-lp-btn-primary" style={s("display:inline-flex;align-items:center;gap:8px;background:#1d4ed8;color:#fff;font-size:15px;font-weight:500;padding:13px 24px;border-radius:11px;text-decoration:none")}>Alle Funktionen entdecken<Arrow /></Link>
            </div>
          </div>
        </section>
        ...
        <section style={s("padding:60px 0 60px")}>
          <div style={s("max-width:1200px;margin:0 auto;padding:0 24px")}>
            <div style={s("max-width:640px;margin:0 auto;text-align:center")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Integrationen</span>
          ...
          <div style={s("max-width:1140px;margin:0 auto")} className="ca-lp-reveal">
            <div className="ca-lp-2col" style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(340px,100%),1fr));gap:clamp(36px,5vw,72px);align-items:center")}>
              <div className="ca-lp-rowtext">
                <span style={s("display:inline-flex;align-items:center;gap:8px;font-family:'Geist Mono',monospace;font-size:11px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#1d4ed8;background:#eef3ff;border:1px solid rgba(37,99,235,0.2);padding:6px 13px;border-radius:999px")}><span style={s("font-weight:700;color:#1d4ed8")}>01</span><span style={s("width:1px;height:11px;background:rgba(37,99,235,0.3)")}></span>Vertragsverwaltung</span>
                ...
                <ul style={s("list-style:none;padding:0;margin:22px 0 0;display:flex;flex-direction:column;gap:13px")}>
                  {[
                    ...
                    <li key={i} style={s("display:flex;align-items:flex-start;gap:11px")}><span style={s("width:30px;height:30px;border-radius:8px;background:rgba(37,99,235,0.08);color:#1d4ed8;display:flex;align-items:center;justify-content:center;flex:none")}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{li.icon}</svg></span><span style={s("font-size:15px;color:#27272a;line-height:1.5")}><strong style={s("color:#0c0c10")}>{li.b}</strong>{li.t}</span></li>
                  ))}
                </ul>
                <Link to={!user ? "/register" : "/dashboard"} className="ca-lp-btn-primary" style={s("display:inline-flex;align-items:center;gap:8px;margin-top:26px;background:#1d4ed8;color:#fff;font-size:15px;font-weight:500;padding:13px 22px;border-radius:11px;text-decoration:none")}>Jetzt Verträge hochladen<Arrow /></Link>
              </div>
              <div className="ca-lp-rowgfx">
                <div style={s("background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:16px;box-shadow:0 34px 70px -34px rgba(37,99,235,0.32);overflow:hidden")}>
                  <div style={s("display:flex;align-items:center;justify-content:space-between;gap:10px;padding:15px 17px;border-bottom:1px solid rgba(17,17,20,0.06);background:#fbfbfd")}>
                    <div style={s("display:flex;align-items:center;gap:9px")}><span style={s("width:30px;height:30px;border-radius:8px;background:#1d4ed8;display:flex;align-items:center;justify-content:center")}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg></span><span style={s("font-size:14px;font-weight:600;color:#0c0c10")}>Meine Verträge</span></div>
                    <div style={s("display:flex;align-items:center;gap:10px")}><span style={s("font-size:11.5px;color:#71717a;font-family:'Geist Mono',monospace")}>24 Dokumente</span><span style={s("display:inline-flex;align-items:center;gap:5px;background:#1d4ed8;color:#fff;font-size:11.5px;font-weight:600;border-radius:8px;padding:6px 10px")}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>Upload</span></div>
                  </div>
                  <div style={s("display:flex;flex-direction:column;gap:9px;padding:15px 17px")}>
                    {[
                      { name: "Mietvertrag_2026.pdf", meta: "Score 87 · vor 2 Tagen", ic: "#b91c1c", icbg: "#fef2f2", ok: true },
                      { name: "Freelancer_NDA.pdf", meta: "Score 92 · vor 5 Tagen", ic: "#1d4ed8", icbg: "#eef2ff", ok: true },
                      { name: "Mobilfunk_2025.pdf", meta: "Frist in 25 Tagen", ic: "#b45309", icbg: "#fffaeb", ok: false },
                      { name: "Mietvertrag_Büro.pdf", meta: "Score 95 · vor 1 Woche", ic: "#047857", icbg: "#ecfdf3", ok: true },
                      { name: "Kaufvertrag_Kfz.pdf", meta: "Score 81 · vor 2 Wochen", ic: "#1d4ed8", icbg: "#eef2ff", ok: true },
                    ].map((row, i) => (
            ...
            <div className="ca-lp-2col" style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(340px,100%),1fr));gap:clamp(36px,5vw,72px);align-items:center")}>
              <div className="ca-lp-rowgfx" style={s("order:1;display:flex;flex-direction:column;align-items:center;gap:18px")}>
                <div style={s("width:100%;max-width:330px;background:#fff;border:1px solid rgba(17,17,20,0.07);border-radius:13px;padding:16px")}>
                  <div style={s("display:flex;align-items:center;gap:9px;margin-bottom:13px")}><span style={s("width:30px;height:30px;border-radius:8px;background:#1d4ed8;display:flex;align-items:center;justify-content:center")}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path></svg></span><div><div style={s("font-size:12.5px;font-weight:600;color:#0c0c10")}>Neue E-Mail</div><div style={s("font-size:11px;color:#71717a;font-family:'Geist Mono',monospace")}>an: upload@contract-ai.de</div></div></div>
                  <div style={s("display:flex;align-items:center;gap:9px;background:#f6f6fa;border:1px dashed rgba(37,99,235,0.3);border-radius:9px;padding:10px 12px")}><span style={s("width:26px;height:26px;border-radius:6px;background:#fef2f2;display:flex;align-items:center;justify-content:center;color:#b91c1c;flex:none")}><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg></span><span style={s("font-size:12.5px;color:#27272a;font-weight:500")}>arbeitsvertrag.pdf</span></div>
                </div>
                <span style={s("width:34px;height:34px;border-radius:50%;background:#1d4ed8;display:flex;align-items:center;justify-content:center;box-shadow:0 8px 18px rgba(37,99,235,0.4)")}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14"></path><path d="m19 12-7 7-7-7"></path></svg></span>
                <div style={s("width:100%;max-width:330px;display:flex;align-items:center;gap:11px;background:#fff;border:1px solid rgba(16,185,129,0.3);border-radius:11px;padding:13px")}><span style={s("width:34px;height:34px;border-radius:50%;background:#ecfdf3;display:flex;align-items:center;justify-content:center;flex:none")}><Check stroke="#10b981" w={18} /></span><div><div style={s("font-size:13.5px;font-weight:600;color:#0c0c10")}>Automatisch erfasst &amp; analysiert</div><div style={s("font-size:11.5px;color:#047857;font-weight:500")}>Jetzt im Dashboard verfügbar</div></div></div>
              </div>
              <div className="ca-lp-rowtext" style={s("order:2")}>
                <span style={s("display:inline-flex;align-items:center;gap:8px;font-family:'Geist Mono',monospace;font-size:11px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#1d4ed8;background:#eef3ff;border:1px solid rgba(37,99,235,0.2);padding:6px 13px;border-radius:999px")}><span style={s("font-weight:700;color:#1d4ed8")}>02</span><span style={s("width:1px;height:11px;background:rgba(37,99,235,0.3)")}></span>E-Mail Upload</span>
                ...
                <Link to="/features/email-upload" className="ca-lp-textlink" style={s("display:inline-flex;align-items:center;gap:7px;margin-top:24px;font-size:15px;font-weight:500;color:#1d4ed8;text-decoration:none")}>E-Mail Upload entdecken<Arrow /></Link>
              </div>
            </div>
          </div>
        </section>
        ...
        <section style={s("padding:56px 24px")}>
          <div style={s("max-width:1200px;margin:0 auto")}>
            <div style={s("max-width:720px;margin:0 auto;text-align:center")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Funktionen</span>
            ...
            {showcase.map((row, i) => (
              <div key={i} style={s(`display:grid;grid-template-columns:repeat(auto-fit,minmax(min(440px,100%),1fr));gap:48px;align-items:center;margin-top:${i === 0 ? "64px" : "72px"}`)} className="ca-lp-reveal ca-lp-2col">
                ...
                <div className="ca-lp-rowgfx" style={s(`${row.reverse ? "order:1;" : ""}background:#fff;border:1px solid rgba(17,17,20,0.09);border-radius:16px;box-shadow:0 30px 60px -26px rgba(17,17,20,0.24);overflow:hidden`)}>
                  <div style={s("height:40px;display:flex;align-items:center;gap:7px;padding:0 14px;border-bottom:1px solid rgba(17,17,20,0.06);background:#fbfbfd")}><span style={s("font-size:12px;color:#71717a;font-family:'Geist Mono',monospace")}>{row.label}</span></div>
        ...
        <section style={s("padding:64px 24px")}>
          <div style={s("max-width:1200px;margin:0 auto")}>
            <div style={s("text-align:center;max-width:600px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Und vieles mehr</span>
            ...
            <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(300px,100%),1fr));gap:14px;margin-top:40px")} className="ca-lp-stagger">
              {tools.map((t, i) => (
                <Link key={i} to={t.to} className="ca-lp-toolcard" style={s("display:flex;align-items:center;gap:14px;background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:14px;padding:18px;text-decoration:none")}>
                  <span style={s("width:40px;height:40px;border-radius:10px;background:rgba(37,99,235,0.08);color:#1d4ed8;display:flex;align-items:center;justify-content:center;flex:none")}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{t.icon}</svg></span>
            ...
            <div style={s("text-align:center;margin-top:30px")} className="ca-lp-reveal">
              <Link to="/features" className="ca-lp-textlink" style={s("display:inline-flex;align-items:center;gap:6px;font-size:14px;font-weight:500;color:#1d4ed8;text-decoration:none")}>Alle Funktionen ansehen<Arrow w={15} /></Link>
            </div>
          </div>
        </section>
        ...
        <section id="types" style={s("padding:72px 24px;position:relative;overflow:hidden;isolation:isolate")}>
          ...
          <div style={s("max-width:1200px;margin:0 auto")}>
            <div style={s("text-align:center;max-width:680px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Vertragstypen</span>
        ...
        <section id="voices" style={s("padding:64px 0 72px;position:relative;overflow:hidden;isolation:isolate")}>
          <div style={s("max-width:1200px;margin:0 auto;padding:0 24px")}>
            <div style={s("text-align:center;max-width:640px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Kundenstimmen</span>
              ...
              <div style={s("display:inline-flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:16px 22px;margin-top:24px;padding:14px 22px;background:#fff;border:1px solid rgba(17,17,20,0.08);border-radius:16px;box-shadow:0 10px 30px -18px rgba(17,17,20,0.18)")}>
                ...
                <div style={s("display:flex;align-items:center;gap:8px;font-size:13.5px;color:#3f3f46;font-weight:500")}><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#047857" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>98% würden uns weiterempfehlen</div>
              </div>
            </div>
          </div>
        ...
        <section style={s("padding:64px 24px")}>
          <div style={s("max-width:880px;margin:0 auto;background:#f4f4f7;border:1px solid rgba(17,17,20,0.07);border-radius:26px;padding:clamp(36px,5vw,64px);position:relative;overflow:hidden")} className="ca-lp-reveal">
            ...
            <div style={s("position:relative;text-align:center")}>
              ...
              <div style={s("display:inline-flex;align-items:center;gap:9px;margin-top:26px;background:#ecfdf3;border:1px solid #c8f0d8;border-radius:999px;padding:10px 20px")}><Check stroke="#047857" w={18} /><span style={s("font-size:15px;font-weight:600;color:#065f46")}>Contract&nbsp;AI. Automatisch. 24/7.</span></div>
            </div>
          </div>
        </section>
        ...
        <section style={s("padding:64px 24px;position:relative;overflow:hidden;isolation:isolate")}>
          <div style={s("max-width:980px;margin:0 auto")}>
            <div style={s("text-align:center;max-width:640px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Im Vergleich</span>
            ...
            <div style={s("margin-top:44px;overflow-x:auto")} className="ca-lp-reveal ca-lp-cmp-desktop">
              <div className="ca-lp-cmp" style={s("min-width:640px;border:1px solid rgba(17,17,20,0.08);border-radius:18px;overflow:hidden;background:#fff;box-shadow:0 30px 60px -40px rgba(17,17,20,0.2)")}>
                <div style={s("display:grid;grid-template-columns:1.6fr 1fr 1fr 1fr;background:#fafafb;border-bottom:1px solid rgba(17,17,20,0.08)")}>
                  ...
                  <div style={s("padding:18px 14px;text-align:center;background:linear-gradient(180deg,rgba(37,99,235,0.1),rgba(37,99,235,0.04));border-left:1px solid rgba(37,99,235,0.18);border-right:1px solid rgba(37,99,235,0.18)")}><span style={s("display:inline-flex;align-items:center;gap:6px;font-size:14px;font-weight:700;color:#1d4ed8")}><svg width="15" height="15" viewBox="0 0 28 28"><rect width="28" height="28" rx="8" fill="#1d4ed8"></rect><path d="M8 14.4l3.8 3.8L20 9.2" stroke="#fff" strokeWidth="2.6" fill="none" strokeLinecap="round" strokeLinejoin="round"></path></svg>Contract&nbsp;AI</span></div>
        ...
        <section id="pricing" style={s("padding:64px 24px 80px;position:relative;overflow:hidden;isolation:isolate")}>
          ...
          <div style={s("max-width:1200px;margin:0 auto")}>
            <div style={s("text-align:center;max-width:640px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>Preise</span>
              ...
              <div style={s("display:inline-flex;align-items:center;gap:4px;margin-top:28px;padding:5px;background:#f1f1f5;border-radius:13px;position:relative")}>
                ...
                <span style={s("margin-left:8px;margin-right:6px;display:inline-flex;align-items:center;gap:4px;font-size:11.5px;font-weight:600;color:#047857;background:#ecfdf3;border:1px solid #c8f0d8;border-radius:999px;padding:4px 10px")}>2 Monate gratis</span>
              </div>
            </div>
            <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(min(290px,100%),1fr));gap:20px;margin-top:48px;align-items:stretch")} className="ca-lp-stagger ca-lp-pricegrid">
              ...
              <div className="ca-lp-pricecard" style={s("background:#fff;border:1px solid rgba(17,17,20,0.1);border-radius:20px;padding:32px;display:flex;flex-direction:column")}>
                ...
                <div style={s("display:flex;align-items:baseline;gap:4px;margin:22px 0 0")}><span style={s("font-size:44px;font-weight:700;letter-spacing:-0.03em;color:#0c0c10")}>0€</span><span style={s("font-size:14px;color:#71717a")}>/ Monat</span></div>
              ...
              <div className="ca-lp-pricecard" style={s("background:#fff;border:1px solid rgba(17,17,20,0.1);border-radius:20px;padding:32px;display:flex;flex-direction:column")}>
                ...
                <div style={s("display:flex;align-items:flex-end;gap:8px;margin:22px 0 0")}>
                  ...
                  <span style={s("font-size:14px;color:#71717a;padding-bottom:5px")}>/ Monat</span>
                  <span style={s("font-size:17px;color:#71717a;text-decoration:line-through;padding-bottom:6px")}>{biz.old}€</span>
                </div>
                <div style={s("display:flex;align-items:center;gap:8px;margin-top:10px;min-height:24px")}>
                  <span style={s("font-size:12.5px;color:#71717a")}>{biz.note}</span>
                  {biz.save && <span style={s("display:inline-flex;align-items:center;font-size:11.5px;font-weight:600;color:#047857;background:#ecfdf3;border:1px solid #c8f0d8;border-radius:999px;padding:3px 9px")}>{biz.save}</span>}
                </div>
                ...
                <p style={s("font-size:11.5px;color:#71717a;text-align:center;margin:9px 0 0")}>Keine Kreditkarte nötig</p>
                <div style={s("height:1px;background:rgba(17,17,20,0.07);margin:24px 0")}></div>
                <ul style={s("list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:12px")}>
              ...
              <div className="ca-lp-pricecard-pop" style={s("background:linear-gradient(180deg,#f5f8ff,#ffffff);border:2px solid #1d4ed8;border-radius:20px;padding:32px;display:flex;flex-direction:column;position:relative;box-shadow:0 30px 60px -24px rgba(37,99,235,0.35)")}>
                <span style={s("position:absolute;top:20px;right:20px;font-family:'Geist Mono',monospace;font-size:10.5px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:#fff;background:#1d4ed8;border-radius:999px;padding:5px 11px;display:inline-flex;align-items:center;gap:5px")}><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26"></polygon></svg>Meist gewählt</span>
                ...
                <div style={s("display:flex;align-items:flex-end;gap:8px;margin:22px 0 0")}>
                  ...
                  <span style={s("font-size:14px;color:#71717a;padding-bottom:5px")}>/ Monat</span>
                  <span style={s("font-size:17px;color:#71717a;text-decoration:line-through;padding-bottom:6px")}>{ent.old}€</span>
                </div>
                <div style={s("display:flex;align-items:center;gap:8px;margin-top:10px;min-height:24px")}>
                  <span style={s("font-size:12.5px;color:#71717a")}>{ent.note}</span>
                  {ent.save && <span style={s("display:inline-flex;align-items:center;font-size:11.5px;font-weight:600;color:#047857;background:#ecfdf3;border:1px solid #c8f0d8;border-radius:999px;padding:3px 9px")}>{ent.save}</span>}
                </div>
                <Link to={user ? "/pricing" : "/register?from=pricing&plan=premium"} className="ca-lp-btn-primary" style={s("display:flex;align-items:center;justify-content:center;gap:7px;margin:24px 0 0;background:#1d4ed8;color:#fff;font-size:15px;font-weight:500;padding:13px;border-radius:11px;text-decoration:none")}>Enterprise wählen<Arrow /></Link>
                <p style={s("font-size:11.5px;color:#1d4ed8;text-align:center;margin:9px 0 0;font-weight:500")}>14 Tage Geld-zurück-Garantie</p>
                <div style={s("height:1px;background:rgba(17,17,20,0.07);margin:24px 0")}></div>
                <div style={s("font-size:12px;font-weight:600;color:#1d4ed8;margin:0 0 12px")}>Alles aus Business, plus:</div>
                <ul style={s("list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:12px")}>
            ...
            <div style={s("display:flex;flex-wrap:wrap;gap:14px 28px;justify-content:center;align-items:center;margin-top:32px")} className="ca-lp-reveal">
              <span style={s("display:inline-flex;align-items:center;gap:8px;font-size:14px;color:#3f3f46;font-weight:500")}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#047857" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><path d="m9 12 2 2 4-4"></path></svg>14 Tage Geld-zurück-Garantie</span>
              <span style={s("display:inline-flex;align-items:center;gap:8px;font-size:14px;color:#3f3f46;font-weight:500")}><Check stroke="#047857" w={18} sw="2.2" />Keine Mindestlaufzeit</span>
              <span style={s("display:inline-flex;align-items:center;gap:8px;font-size:14px;color:#3f3f46;font-weight:500")}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#047857" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>In 60 Sekunden startklar</span>
            </div>
          </div>
        </section>
        ...
        <section style={s("padding:72px 24px;position:relative;overflow:hidden;isolation:isolate")}>
          ...
          <div style={s("max-width:780px;margin:0 auto")}>
            <div style={s("text-align:center;max-width:600px;margin:0 auto")} className="ca-lp-reveal">
              <span style={s("font-family:'Geist Mono',monospace;font-size:12px;font-weight:500;letter-spacing:0.14em;text-transform:uppercase;color:#1d4ed8")}>FAQ</span>
            ...
            <div style={s("display:flex;flex-direction:column;gap:12px;margin-top:40px")} className="ca-lp-reveal">
              {faqs.map((f, i) => (
                <details key={i} className="ca-lp-faq" style={s("background:#fff;border:1px solid rgba(17,17,20,0.09);border-radius:14px;padding:4px 20px")}>
                  <summary style={s("display:flex;align-items:center;justify-content:space-between;gap:16px;cursor:pointer;padding:16px 0")}><span className="ca-lp-faq-q" style={s("font-size:16px;font-weight:600;color:#0c0c10;transition:color .2s")}>{f.q}</span><svg className="ca-lp-faq-chev" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1d4ed8" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={s("flex:none")}><polyline points="6 9 12 15 18 9"></polyline></svg></summary>
        ...
        <section style={s("padding:24px 24px 88px")}>
          <div style={s("max-width:1080px;margin:0 auto;background:linear-gradient(135deg,#1e40af,#1d4ed8 42%,#06b6d4);border-radius:32px;padding:clamp(40px,5vw,76px) clamp(28px,4vw,64px);text-align:center;position:relative;overflow:hidden")} className="ca-lp-reveal ca-lp-finalcta">
        ...
        {!user && (
          <Link to="/register" className={"ca-lp-floatcta" + (showFloat ? " ca-in" : "")} style={s("position:fixed;right:24px;bottom:24px;z-index:90;display:inline-flex;align-items:center;gap:8px;background:#1d4ed8;color:#fff;font-size:15px;font-weight:600;padding:14px 22px;border-radius:999px;text-decoration:none;box-shadow:0 14px 34px rgba(37,99,235,0.45);opacity:0;transform:translateY(20px);pointer-events:none")}>Kostenlos starten<Arrow /></Link>
        )}
