...
import { ArrowLeft, FileText, Clock, Trash2, Loader2, ExternalLink, AlertTriangle } from 'lucide-react';
...
export default function OptimizerHistory() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      setLoading(true);
      const data = await apiCall('/api/optimizer-v2/history') as { success: boolean; results: HistoryItem[] };
      if (data?.success) {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    setDeletingId(id);
    try {
      await apiCall(`/api/optimizer-v2/results/${id}`, { method: 'DELETE' });
      ...
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <div className={styles.headerContent}>
            <h1 className={styles.title}>Analyse-Historie</h1>
            <p className={styles.subtitle}>
              Alle bisherigen Vertragsanalysen im Überblick
            </p>
          </div>
          <button className={styles.newBtn} onClick={() => navigate('/optimizer-v2')}>
            Neue Analyse
          </button>
        </div>
