...
export default function V2ActionBar(props: Props) {
  ...
  return (
    <div className={styles.actionBar} role="toolbar" aria-label="Vertrags-Aktionen">
      ...
      >
        {optimizing ? <Loader size={14} /> : <Zap size={14} />}
        <span>{optimizing ? "Optimiere..." : "Vertrag jetzt optimieren"}</span>
        {!optimizing && <ArrowRight size={12} />}
      </button>
      ...
      >
        {generatingPdf ? <Loader size={13} /> : <FileText size={13} />}
      ...
      >
        {openingChat ? <Loader size={13} /> : <MessageSquare size={13} />}
        <span>Chat</span>
