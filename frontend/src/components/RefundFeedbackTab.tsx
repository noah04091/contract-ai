// frontend/src/components/RefundFeedbackTab.tsx
// Admin-Tab für Refund-Feedback — extrahiert aus pages/RefundFeedbackAdmin.tsx,
// damit das Tool im zentralen Admin-Center (AdminDashboard) statt als lose Seite lebt.
...
import styles from "../pages/RefundFeedbackAdmin.module.css";
...
export default function RefundFeedbackTab() {
  const { user } = useAuth();
  ...
  // Load feedbacks (Parent AdminDashboard rendert diesen Tab nur für Admins)
  useEffect(() => {
    if (user?.role === "admin") {
      loadFeedbacks();
    }
