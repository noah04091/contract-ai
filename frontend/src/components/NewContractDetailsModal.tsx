...
const groupCalendarEvents = (events: CalEventLite[], contractName?: string) => {
  ...
  const groups = Array.from(famMap.values()).map((g) => {
    ...
    return {
      family: g.family,
      label: stripFileName(cleanDeadlineName(g.base ? g.base.title : (g.reminders[0]?.title || 'Termin'), contractName)) || 'Termin',
      ...
      <div className={styles.section}>
        ...
        <div className={styles.detailsGrid}>
          ...
          {EDITABLE_FIELDS.map((field) => {
            ...
            if (isContractLifecycleFieldHiddenForDocType(field.key, contract.documentType, contract.documentCategory, contract.contractType) && !isBeingEdited) return null;
            return (
