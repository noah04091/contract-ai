...
**Letzte Aktualisierung:** 2026-06-22 — **Geheimwaffen-Roadmap (4 Features, vom User freigegeben, in Reihenfolge aufsteigendes Risiko):** ① 🛡️ Rechts-Check (KI-Gegenanwalt) ✅ **LIVE** (additiv: `reviewContract`+`POST /premium/review`, ReviewCard mit Badge/Checkliste/„Empfehlungen übernehmen"); ② ⚡ Live-Schreiben (Streaming) ✅ **LIVE** (additive Route `POST /premium/generate-stream` ndjson; `generateContractText(onDelta)`; Frontend `streamGenerate` lässt Vertrag live wachsen; alte `/generate` bleibt Fallback; + Politur: kompakte „Empfehlungen übernehmen"-Blase via `display`); ③ 🔔 Fristen→Kalender ✅ **LIVE** (additiv: `extractContractDates` grounded → `datesToContractFields` → ruft bestehendes `generateEventsForContract(db,contract)` in `/generate-stream`, non-fatal/idempotent; Frontend zeigt „X Fristen übernommen". KEIN Eingriff in Kalender-/Cron-Code. Verifiziert: Mietvertrag→Termine, Kaufvertrag→nichts); ④ ✍️ Gegenseite signieren (größter Brocken, zuletzt). Davor (21.06.): Design-Varianten, kein „Contract AI" im Dokument, Zeichen-Bereinigung, Unterschrift-Zeichenfeld→PDF. Alles deployt, Premium via `?premium=1` testbar.
