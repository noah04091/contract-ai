...
**Letzte Aktualisierung:** 2026-06-22 — **Geheimwaffen-Roadmap (4 Features, vom User freigegeben, in Reihenfolge aufsteigendes Risiko):** ① 🛡️ Rechts-Check (KI-Gegenanwalt) ✅ **LIVE** (additiv: `reviewContract`+`POST /premium/review`, ReviewCard mit Badge/Checkliste/„Empfehlungen übernehmen"); ② ⚡ Live-Schreiben (Streaming) ✅ **LIVE** (additive Route `POST /premium/generate-stream` ndjson; `generateContractText(onDelta)`; Frontend `streamGenerate` lässt Vertrag live wachsen; alte `/generate` bleibt Fallback; + Politur: kompakte „Empfehlungen übernehmen"-Blase via `display`); ③ 🔔 Fristen→Kalender ✅ **LIVE** (additiv: `extractContractDates` grounded → `datesToContractFields` → ruft bestehendes `generateEventsForContract(db,contract)` in `/generate-stream`, non-fatal/idempotent; Frontend zeigt „X Fristen übernommen". KEIN Eingriff in Kalender-/Cron-Code. Verifiziert: Mietvertrag→Termine, Kaufvertrag→nichts); ④ ✍️ Gegenseite signieren ✅ **LIVE** (reine Frontend-Anbindung: „Zur Unterschrift senden" → Premium-PDF → `POST /contracts/:id/upload-pdf` (S3) → öffnet bestehenden `EnhancedSignatureModal` → komplettes vorhandenes Envelope-System: Gegenpartei-Mail, öffentliche `/sign/:token`-Seite ohne Account, OTP, PDF-Versiegelung, Audit, Erinnerungen. KEIN Backend-Eingriff).

**🎉 ROADMAP KOMPLETT** — alle 4 Features live (hinter `?premium=1`). ⚠️ Such-Tools (Glob/Grep) lieferten in dieser Session FALSCHE Negative → für Datei-/Code-Suche Bash nutzen. Davor (21.06.): Design-Varianten, kein „Contract AI" im Dokument, Zeichen-Bereinigung, Unterschrift-Zeichenfeld→PDF. Alles deployt, Premium via `?premium=1` testbar.
